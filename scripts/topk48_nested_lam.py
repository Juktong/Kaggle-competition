...
FEAT=f'{SH}/topk48_feat.csv'; PATHDIR=f'{SH}/topk48_paths'; FOLDS=5; NBOOT=400
...
print(f"  deployed pipeline w/ K=48 PF mean = {base_r:.4f}   (submitted 54844628 used a 24-seed PF: 8.8626)")
print(f"  -> seeds-only effect (24 -> 48, NO ranker) = {8.8626-base_r:+.4f}\n",flush=True)
...
print(f"  NESTED-lambda result = {nested:.4f}   gain vs deployed(K48) {base_r-nested:+.4f}")
...
g=np.array(gains)
print(f"  bootstrap({NBOOT}) gain vs K48 deployed: mean={g.mean():+.4f} 5th={np.percentile(g,5):+.4f} 1st={np.percentile(g,1):+.4f} frac>0={100*np.mean(g>0):.0f}%")
