...
| B | selector / router with ASYMMETRIC confidence | PF-uncertainty (seed spread) + GBM disagreement predict *which* model to trust | **PARTIAL-NEG, full test RUNNING** | `gbm_disagree` refuted at full power (A). `pf_unc` on 96-well partial: corr(eD−eP)=+0.169, a* spread 0.587, BUT **nested router 10.316 LOSES to fixed-0.5 10.263**; under-powered (even a global refit loses there). Full 773-well PF-unc run in progress (detached pid 24489) for the full-power re-test. |
